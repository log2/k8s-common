...
chart_check()
{
    ...
    # shellcheck disable=SC2317 # nested functions are reachable via name, shellcheck can't see it
    isDeployed()
    {
    ...
    if [ -n "$SHOW_CONTENT" ]; then
        ...
        # shellcheck disable=SC2086 # EXTRA_HELM_OPTIONS intentionally unquoted for word splitting
